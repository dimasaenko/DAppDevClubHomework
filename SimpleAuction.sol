...
contract SimpleAuction is AuctionInterface {
    ...
    /**
     * @notice this mapping used for storing seller rating
     */
    ...
    */
     ...
     */
    function createLot(string _name, uint _price, uint _minBid) external {
    ...
    */
     ...
     */
    function rate(uint _lotID, bool _option) lotExists(_lotID) public {
        Lot memory lot = lots[_lotID];
        require(lot.isProcessed && lot.bidder == msg.sender);
        if (_option) {
            rating[lots[_lotID].owner] += 1;
        } else {
            rating[lots[_lotID].owner] -= 1;
        }
    }
     ...
     */
    function getRating(address _owner) constant public returns (uint) {
        return rating[_owner];
    }
}
